...
class Sheet(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    creation = db.Column(db.Date, nullable=False) # Date of entry creation in database
    update = db.Column(db.Date, nullable=False) # Date of entry update in database
    ...
    result = db.relationship('Result', backref='sheet') # O-t-m relation with results
...
class Article(db.Model):
    ...
    title = db.Column(db.String(120), unique=True, nullable=False) #article title
    ...
    # Many-to-many relation created with contributors above
    result = db.relationship('Result', backref='article') # O-t-m relation with results
